...
from convlab.data.multiwoz.info import REF_SYS_DA, REF_USR_DA, init_state


class MultiwozStateEncoder:
                ...
                for slot in request_state[domain]:
                    if slot == "ref":
                        ...
                        try:
                            domain_vector[
                                list(
                                    REF_USR_DA[domain.capitalize()].keys()
                                ).index(slot)
                            ] = 1
        ...
        for domain in domains:
            ...
            for slot in belief_state[domain]["semi"]:
                ...
                if belief_state[domain]["semi"][slot] in ["", "not mentioned"]:
                    ...
                    belief_state[domain]["semi"][slot] == "dont care"
                    or belief_state[domain]["semi"][slot] == "dontcare"
                    or belief_state[domain]["semi"][slot] == "don't care"
                ):
                    slot_enc[1] = 1
                    domain_active = True
        ...
        if "book" in belief_state["restaurant"]:
            if "booked" in belief_state["restaurant"]["book"]:
                if belief_state["restaurant"]["book"]["booked"]:
                    if (
                        "reference"
                        in belief_state["restaurant"]["book"]["booked"][0]
                    ):
        ...
        if "book" in belief_state["hotel"]:
            if "booked" in belief_state["hotel"]["book"]:
                if belief_state["hotel"]["book"]["booked"]:
                    if (
                        "reference"
                        in belief_state["hotel"]["book"]["booked"][0]
                    ):
        ...
        if "book" in belief_state["train"]:
            if "booked" in belief_state["train"]["book"]:
                if belief_state["train"]["book"]["booked"]:
                    if (
                        "reference"
                        in belief_state["train"]["book"]["booked"][0]
                    ):
