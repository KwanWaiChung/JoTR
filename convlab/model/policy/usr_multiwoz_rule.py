...
from convlab.data.multiwoz.goal_generator import GoalGenerator
...
class UserPolicyAgendaMultiWoz:
    """ The rule-based user policy model by agenda. Derived from the UserPolicy class """

    # load stand value
    stand_value_dict = json.load(
        open(
            os.path.join(
                os.path.dirname(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                ),
                "data/multiwoz/value_set.json",
            )
        )
    )

    def __init__(self, max_turn=20, domains=None):
        """
        ...
        self.goal_generator = GoalGenerator(domains=domains)
    ...
    def predict(self, sys_action: Dict[str, List[List[str]]]):
        """
        Predict an user act based on state and preorder system action.
        Args:
            sys_action (Dict): System dialogue act. Note that the typical
                input of a policy is state. This rule policy is a special
                case and requires special attention. 
    ...
    def get_in_reward(self, domain):
        ...
        if (
            ...
            if "reqt" in self.domain_goals[domain.lower()]:
                ...
                if all([val not in NOT_SURE_VALS for val in reqt_vals]):
                    complete = True
                    new = 40.0
            ...
            if "general" not in act:
                ...
                if dom in REF_SYS_DA_M.keys():
                    ...
                    for pairs in action[act]:
                        ...
                        if (
                            REF_SYS_DA_M[dom].get(pairs[0].lower(), None)
                            is not None
                        ):
                            new_list.append(
                                [
                                    REF_SYS_DA_M[dom][pairs[0].lower()],
        ...
        if intent in ["book", "inform"]:
            ...
            for [slot, value] in slot_vals:
                ...
                if slot in g_reqt:
                    ...
                    self._push_item(
                        domain + "-inform", slot, g_fail_info[slot]
                    )
                    info_right = False
                elif (
                    len(g_fail_info) <= 0
                    ...
                    self._push_item(
                        domain + "-inform", slot, g_fail_book[slot]
                    )
                    info_right = False
                elif (
                    len(g_fail_book) <= 0
                    and slot in g_book
                    and value != g_book[slot]
                ):
            ...
            if len(g_fail_book) > 0:
                ...
                for slot in g_book.keys():
                    if (slot not in g_fail_book) or (
                        slot in g_fail_book
                        and g_fail_book[slot] != g_book[slot]
                    ):
                        self._push_item(domain + "-inform", slot, g_book[slot])
                ...
                goal.domain_goals[domain][
                    "fail_book_fail"
                ] = goal.domain_goals[domain].pop("fail_book")
            ...
            for [slot, _] in slot_vals:
                ...
                if slot in g_reqt:
                    ...
                    if domain == "taxi" and (
                        ...
                        places = [
                            dom
                            for dom in goal.domains[
                                : goal.domains.index("taxi")
                            ]
                            if "address" in goal.domain_goals[dom]["reqt"]
                        ]

                        if (
                            len(places) >= 1
                            and slot == "destination"
                            and goal.domain_goals[places[-1]]["reqt"][
                                "address"
                            ]
                            not in NOT_SURE_VALS
                        ):
                            self._push_item(
                                domain + "-inform",
                                slot,
                                goal.domain_goals[places[-1]]["reqt"][
                                    "address"
                                ],
                            )
                            ...
                            len(places) >= 2
                            and slot == "departure"
                            and goal.domain_goals[places[-2]]["reqt"][
                                "address"
                            ]
                            not in NOT_SURE_VALS
                        ):
                            self._push_item(
                                domain + "-inform",
                                slot,
                                goal.domain_goals[places[-2]]["reqt"][
                                    "address"
                                ],
                            )
                            ...
                            self._push_item(
                                domain + "-inform", slot, DEF_VAL_DNC
                            )
            ...
            if len(g_fail_info) > 0:
                ...
                for slot in g_info.keys():
                    if (slot not in g_fail_info) or (
                        slot in g_fail_info
                        and g_fail_info[slot] != g_info[slot]
                    ):
                        self._push_item(domain + "-inform", slot, g_info[slot])
                ...
                goal.domain_goals[domain][
                    "fail_info_fail"
                ] = goal.domain_goals[domain].pop("fail_info")
