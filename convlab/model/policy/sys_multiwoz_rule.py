...
class RuleBasedMultiwozBot:
    ...
    def __init__(self, seeder: Dict[str, Any] = {}):
        # trick to avoid passing state_dict in RL training loop
        self.last_state = {}
        self.seeder = seeder
    ...
    def _update_greeting(self, user_act, state, DA):
        ...
        if intent_type == "bye":
            if "general-bye" not in DA:
                DA["general-bye"] = []
            if self.seeder.get("py", random).random() < 0.3:
    ...
    def _book_taxi(self, user_act, state, DA):
        ...
        if len(blank_info) == 0:
            if "Taxi-Inform" not in DA:
                DA["Taxi-Inform"] = []
            car = generate_car(self.seeder)
            phone_num = generate_phone_num(11, self.seeder)
        ...
        request_num = (
            self.seeder.get("py", random).randint(0, 999999) % len(blank_info)
            + 1
        )
        ...
        if intent_type == "Request":
            ...
            if len(kb_result) == 0:
                ...
                for slot in state["belief_state"][domain.lower()]["semi"]:
                    if (
                        ...
                        != ""
                        and state["belief_state"][domain.lower()]["semi"][slot]
                        != "do n't care"
                    ):
                ...
                p = self.seeder.get("py", random).random()

                # Ask user if he wants to change constraint
                if p < 0.3:
                    req_num = min(
                        self.seeder.get("py", random).randint(0, 999999)
                        % len(DA[domain + "-NoOffer"])
                        + 1,
                        3,
                    )
            ...
            # elif len(state['kb_results_dict']) == 1:
                ...
                if property_num > 0:
                    ...
                    self.seeder.get("py", random).shuffle(props)
            ...
            # There are multiple resultes matching user's constraint
                ...
                if True:  # p < 0.3:
                    ...
                    idx = self.seeder.get("py", random).randint(
                        0, 999999
                    ) % len(kb_result)
                    # idx = 0
                    ...
                    prop_num = min(
                        self.seeder.get("py", random).randint(0, 999999) % 3,
                        len(props),
                    )
                ...
                # Ask user for more constraint
                    ...
                    while i < len(reqs):
                        if reqs[i][0] not in REQUESTABLE_SLOTS:
                            ...
                            i -= 1
                        i += 1
                    self.seeder.get("py", random).shuffle(reqs)
                    if len(reqs) == 0:
                        return
                    req_num = min(
                        self.seeder.get("py", random).randint(0, 999999)
    ...
    if last_state == {}:
        ...
        for domain in state["belief_state"]:
            ...
            for slot in state["belief_state"][domain]["semi"]:
                if (
                    state["belief_state"][domain]["semi"][slot]
                    != last_state["belief_state"][domain]["semi"][slot]
...
def generate_phone_num(length, seeder={}):
