...
from convlab.data.multiwoz.dbquery import dbs
from convlab.env.evaluator import Evaluator
...
class MultiwozEvaluator(Evaluator):
        ...
        for dom_int in da_turn:
            ...
            for slot, value in slot_pair:
                ...
                if da == "booking-book-ref" and self.cur_domain in [
                    ...
                    if (
                        not self.booked[self.cur_domain]
                        and re.match(r"^\d{8}$", value)
                        and int(value) < len(dbs[self.cur_domain])
                    ):
                        self.booked[self.cur_domain] = dbs[self.cur_domain][
                            int(value)
                        ]
                elif da == "train-offerbooked-ref" or da == "train-inform-ref":
                    if (
                        not self.booked["train"]
                        and re.match(r"^\d{8}$", value)
                        and int(value) < len(dbs["train"])
                    ):
                ...
                for k, v in goal[domain]["info"].items():
                    if k in ["destination", "departure", "name"]:
                        ...
                        try:
                            ...
                            v_select = int(
                                entity["arriveBy"].split(":")[0]
                            ) * 100 + int(entity["arriveBy"].split(":")[1])
                            if v_constraint >= v_select:
                                match += 1
                        except (ValueError, IndexError):
                            match += 1
    ...
    def _inform_F1_goal(self, goal, sys_history, domains=None):
        ...
        for da in sys_history:
            ...
            if (
                intent in ["inform", "recommend", "offerbook", "offerbooked"]
                and domain in domains
                and slot in mapping[domain]
                and value.strip() not in NUL_VALUE
            ):
    ...
    def book_rate(self, ref2goal=True, aggregate=True):
        if ref2goal:
            ...
            for da in self.usr_da_array:
                d, i, s, v = da.split("-", 3)
                if (
                    i in ["inform", "recommend", "offerbook", "offerbooked"]
                    and s in mapping[d]
                ):
    ...
    def task_success(self, ref2goal=True):
        ...
        if (
            (book_sess == 1 and inform_sess[1] == 1)
    ...
    def domain_success(self, domain: str, ref2goal=True):
        """
