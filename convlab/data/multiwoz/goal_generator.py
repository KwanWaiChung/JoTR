...
def nomial_sample(counter: Counter):
    return list(counter.keys())[
        np.argmax(np.random.multinomial(1, list(counter.values())))
    ]
...
class GoalGenerator:
        ...
        for d in dialogs:
            ...
            for domain in d_domains:
                message = (
                    ...
                    if isinstance(dialogs[d]["goal"]["message"], str)
    ...
    def _get_domain_goal(self, domain):
        ...
        while True:
            ...
            if (
                "book" in cnt_slot
                and random.random() < pro_book + pro_correction["book"]
            ):
            ...
            if "book" in domain_goal and random.random() < 0.5:
                if domain == "hotel":
                    ...
                    if "time" in domain_goal["book"] and random.random() < 0.5:
                        ...
                        domain_goal["fail_book"]["time"] = (
                            str((int(hour) + 1) % 24) + ":" + minute
                        )
                    elif "day" in domain_goal["book"]:
            ...
            if (
                "info" in domain_goal
                and len(query(domain, domain_goal["info"].items())) == 0
            ):
                num_trial = 0
                while num_trial < 100:
    ...
    def get_user_goal(self, seed=None):
        ...
        if "taxi" in domain_ordering:
            ...
            if len(places) >= 1:
                ...
                user_goal[places[-1]]["reqt"] = list(
                    set(user_goal[places[-1]].get("reqt", [])).union(
                        {"address"}
                    )
                )
                if (
            ...
            if len(places) >= 2:
                ...
                user_goal[places[-2]]["reqt"] = list(
                    set(user_goal[places[-2]].get("reqt", [])).union(
                        {"address"}
                    )
                )
        ...
        if (
            ...
            if (
                len(query("restaurant", adjusted_restaurant_goal.items())) > 0
            ...
            if dom == "taxi" and len(state[info]) == 1:
                ...
                if len(places) >= 2:
                    ...
                    if "arriveBy" in state[info]:
                        m.append(
                            "The taxi should arrive at the {} from the {} by {}.".format(
                        ...
                        m.append(
                            "The taxi should leave from the {} to the {} after {}.".format(
                                self.boldify(places[0]),
                                self.boldify(places[1]),
                    ...
                    if slot in state[book]:
                        if slot == "people":
                            ...
                            if same_people_domain is None:
                                ...
                                slot_strings.append(
                                    self.boldify(
                                        "for the same group of people as the {} booking".format(
                                            same_people_domain
                                        )
                                    )
                                )
                            ...
                            if same_people_domain is None:
                                ...
                                slot_strings.append(
                                    self.boldify(
                                        "on the same day as the {} booking".format(
                                            same_people_domain
                                        )
                                    )
                                )
...
if __name__ == "__main__":
    goal_generator = GoalGenerator(
        corpus_path="data/multiwoz/annotated_user_da_with_span_full.json"
    )
    while True:
