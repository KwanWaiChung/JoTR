...
goal_path = os.path.join(
    ...
    "data/multiwoz/goal/new_goal_model.pkl",
)
...
class GoalGenerator:
                ...
                if "info" in dialogs[d]["goal"][domain]:
                    ...
                    for slot in dialogs[d]["goal"][domain]["info"]:
                        ...
                        ind_slot_value_cnt[domain]["info"][slot][
                            dialogs[d]["goal"][domain]["info"][slot]
                        ] += 1
    ...
    def _get_domain_goal(self, domain):
        ...
        while True:
            ...
            # if 'book' in domain_goal and self.seeder.get('py', random).random() < 0.5:
            #     if domain == 'hotel':
            #         domain_goal['fail_book'] = deepcopy(domain_goal['book'])
            #         if 'stay' in domain_goal['book'] and self.seeder.get('py', random).random() < 0.5:
            #             # increase hotel-stay
            ...
            #         domain_goal['fail_book'] = deepcopy(domain_goal['book'])
            #         if 'time' in domain_goal['book'] and self.seeder.get('py', random).random() < 0.5:
            #             hour, minute = domain_goal['book']['time'].split(':')
            #             domain_goal['fail_book']['time'] = str((int(hour) + 1) % 24) + ':' + minute
            #         elif 'day' in domain_goal['book']:
            #             if self.seeder.get('py', random).random() < 0.5:
            #                 domain_goal['fail_book']['day'] = days[(days.index(domain_goal['book']['day']) - 1) % 7]
            #             else:
            #                 domain_goal['fail_book']['day'] = days[(days.index(domain_goal['book']['day']) + 1) % 7]
    ...
    def get_user_goal(self):
        ...
        if "taxi" in domain_ordering:
            places = [
                dom
                for dom in domain_ordering[: domain_ordering.index("taxi")]
                if dom
                in ["attraction", "hotel", "restaurant", "police", "hospital"]
            ]
            if len(places) >= 1:
                ...
                if (
                    places[-1] == "restaurant"
                    and "book" in user_goal["restaurant"]
                ):
        ...
        if (
            "restaurant" in domain_ordering
            and "attraction" in domain_ordering
            and "fail_info" not in user_goal["restaurant"]
            and domain_ordering.index("restaurant")
            > domain_ordering.index("attraction")
            ...
            if (
                len(
                    self.db.query(
                        "restaurant", adjusted_restaurant_goal.items()
                    )
                )
                > 0
                and self.seeder.get("py", random).random() < 0.5
            ):
        ...
        if (
            "restaurant" in domain_ordering
            and "hotel" in domain_ordering
            and "book" in user_goal["restaurant"]
            and "book" in user_goal["hotel"]
        ):
            if self.seeder.get("py", random).random() < 0.5:
            ...
            if self.seeder.get("py", random).random() < 1.0:
                ...
                if "fail_book" in user_goal["restaurant"]:
                    ...
                    if (
                        user_goal["restaurant"]["book"]["day"]
                        == user_goal["restaurant"]["fail_book"]["day"]
                        and user_goal["restaurant"]["book"]["time"]
                        == user_goal["restaurant"]["fail_book"]["time"]
                        and user_goal["restaurant"]["book"]["people"]
                        == user_goal["restaurant"]["fail_book"]["people"]
                    ):
                        del user_goal["restaurant"]["fail_book"]

        # match day and people of hotel and train
        if (
            "hotel" in domain_ordering
            and "train" in domain_ordering
            and "book" in user_goal["hotel"]
            and "info" in user_goal["train"]
        ):
            if (
                user_goal["train"]["info"]["destination"] == "cambridge"
                and "day" in user_goal["hotel"]["book"]
            ):
                user_goal["train"]["info"]["day"] = user_goal["hotel"]["book"][
                    "day"
                ]
            elif (
                user_goal["train"]["info"]["departure"] == "cambridge"
                and "day" in user_goal["hotel"]["book"]
                and "stay" in user_goal["hotel"]["book"]
            ):
                user_goal["train"]["info"]["day"] = days[
                    (
                        days.index(user_goal["hotel"]["book"]["day"])
                        + int(user_goal["hotel"]["book"]["stay"])
                    )
                    % 7
                ]
            # In case, we have no query results with adjusted train goal, we simply drop the train goal.
            if (
                len(self.db.query("train", user_goal["train"]["info"].items()))
                == 0
            ):
                del user_goal["train"]
    ...
    def _adjust_info(self, domain, info):
        ...
        adjusted_info[slot] = self.seeder.get("py", random).choice(
...
if __name__ == "__main__":
    goal_generator = GoalGenerator(
        corpus_path=os.path.join(get_root_path(), "data/multiwoz/train.json"),
