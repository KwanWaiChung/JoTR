...
from prefinetune import get_response_model

from convlab.model.state_encoder.dagpt_state_encoder import DAGPTStateEncoder
from convlab.model.action_encoder.dagpt_action_encoder import (
    DAGPTActionEncoder3,
)
from convlab.dialog_agent.agent import PipelineAgent
from convlab.model.dst.multiwoz_rule_dst import RuleDST
...
from convlab.data.multiwoz.info import (
    REF_SYS_DA,
)
from env import AggressiveRewardEnv, ConservativeRewardEnv
...
def get_args():
    ...
    parser.add_argument("--rew_weight", type=float, default=1)
...
class PPOTrainer:
    ...
    def train(self, total_timesteps: int, n_warmup_timesteps: int):
        ...
        # while self.t_so_far < n_warmup_timesteps:  # ALG STEP 2
        #     # ALG STEP 3
        #     collected_outputs: Dict[str, Any] = self.warmup_rollout(
        #         step_per_collect=config["step_per_collect"]
        #     )

        #     # Calculate how many timesteps we collected this batch
        #     self.t_so_far += collected_outputs["ep_len"].sum().item()
        ...
        #     avg_succ = np.mean(
        #         [i["success"] for i in collected_outputs["ep_info"]]
        #     ).item()
        #     avg_len = collected_outputs["ep_len"].mean().item()
        #     avg_rew = collected_outputs["ep_rew"].mean().item()
        #     log_dict = {
        #         "warmup/avg_succ": avg_succ,
        #         "warmup/avg_rew": avg_rew,
        #         "warmup/avg_len": avg_len,
        #         "trainer/env_step": self.t_so_far,
        #         "memory(MB)": torch.cuda.memory_allocated() / 1024 / 1024,
        #     }
        #     self.global_outputs.append(log_dict)
        #     bar.update(int(collected_outputs["ep_len"].sum().item()))
        #     bar.set_postfix(
        #         {
        #             "warmup/avg_succ": f"{avg_succ:.2f}",
        #             "warmup/avg_rew": f"{avg_rew:.2f}",
        #             "warmup/avg_len": f"{avg_len:.2f}",
        #         }
        #     )
        #     if config["use_wandb"]:
        #         wandb.log(log_dict)
        #     self.update(collected_outputs)
        ...
        while t_env < step_per_collect and n_ep < episode_per_collect:
            ...
            for t in range(config["max_turn"]):
                ...
                response_ids, label_ids, cat_ids = str_to_ids(
                    ...
                    domains=DialogActTurnDataset.domains,
                    intents=DialogActTurnDataset.intents,
                    slots=DialogActTurnDataset.slots,
                )
...
def get_env(
    ...
    action_encoder = DAGPTActionEncoder3(
    ...
    state_encoder = DAGPTStateEncoder(
    ...
    policy_usr = RulePolicy(
        character="usr", max_turn=config["max_turn"], seeder=seeder
    )
    ...
    if config["rew_type"] == "normal":
        ...
        env = AggressiveRewardEnv(
            ...
            slot_mapping=REF_SYS_DA,
            rew_weight=config["rew_weight"],
        )
        ...
        env = ConservativeRewardEnv(
            ...
            evaluator=evaluator,
            encode_state=True,
            inform_intents=["Inform", "Recommend"],
            request_intents=["Request"],
            rew_weight=config["rew_weight"],
        )
    return env
...
def test(config):
    ...
    if "context_model_name" not in config_saved:
        from prefinetune import get_context_model

        context_model = get_context_model(
            config_saved, remove_dropout=config["remove_dropout"]
        )
    # Use below for prefinetun2_scratch
    # context_model = get_context_model(
    #     config_saved, remove_dropout=config["remove_dropout"]
    # )
    # end comment
...
def train(config):
    ...
    if "context_model_name" in config_saved:
